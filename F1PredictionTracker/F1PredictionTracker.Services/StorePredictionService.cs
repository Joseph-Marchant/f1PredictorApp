    ...
    IRetrievePredictionStandings retrievePredictionStandings)
{
    ...
    public string StoreDefaultPrediction(string userName, List<string> prediction)
    {
        if (prediction.Count != 3)
        {
            throw new ArgumentException("Invalid number of predictions, must be three");
        ...
        var standings = retrievePredictionStandings.GetPredictionStandings();
        var user = standings.Users.FirstOrDefault(s => s.Name == userName) ?? new User(userName);
        var existingDefaultPrediction = retrievePredictions.GetDefaultPrediction(user);
        var newDefaultPrediction = new Prediction(userName, prediction[0].ToUpper(), prediction[1].ToUpper(), prediction[2].ToUpper());
        storePredictions.StoreDefaultPrediction(newDefaultPrediction, user);
        var predictionString = $"{userName}'s prediction of {prediction[0].ToPascalCase()}{prediction[1].ToPascalCase()}{prediction[2].ToPascalCase()}";
        if (existingDefaultPrediction is not null)
        {
            return $"{predictionString} has been successfully updated";
        }
        return $"{predictionString} has been successfully stored";
    }

    private IList<Prediction> GetPredictions(string userName, List<string> prediction)
    {
        if (prediction.Count != 3)
        {
            throw new ArgumentException("Invalid number of predictions, must be three");
